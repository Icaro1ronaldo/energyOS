...
export default function MapPage({ startDate, endDate }: Props) {
  ...
  // ── Heatmap: always fetch the last 8 days regardless of user date selection ──
  // Hindcast data only exists for the most recent N days (generated by the ML
  // worker at run time). A user may have selected a historical range that has no
  // hindcast rows at all, leaving the heatmap empty. Fetching a fixed trailing
  // window guarantees we cover the period where hindcast data actually lives.
  const heatmapStartIso = useMemo(() => subDays(new Date(), 8).toISOString(), []);
  const heatmapEndIso   = useMemo(() => new Date().toISOString(), []);
  const { data: hmPrices, loading: hmLoadingP } = useMultiZonePrices(DATA_ZONES, heatmapStartIso, heatmapEndIso);
  const { data: hmLoad,   loading: hmLoadingL } = useMultiZoneLoad(DATA_ZONES, heatmapStartIso, heatmapEndIso);
  ...
  return (
    <div style={{ padding: "32px 32px 24px", maxWidth: 1400, margin: "0 auto" }}>
      ...
      <div style={{ ...CARD }}>
        ...
        {metric !== "production" ? (
          <MaeHeatmap
            zoneData={metric === "prices" ? hmPrices : hmLoad}
            zones={selectedZones.length ? selectedZones : DATA_ZONES}
            loading={metric === "prices" ? hmLoadingP : hmLoadingL}
